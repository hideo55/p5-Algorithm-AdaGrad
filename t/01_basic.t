...
subtest 'simple' => sub {
    ...
    $ag->update( [ { label => 1, data => { "a" => 1.0, } }, { label => -1, data => { "b" => 1.5, } }, ] );
...
subtest 'exception' => sub {
    ...
    like exception { $ag->update( [ { data => { "a" => 1.0, } } ] ); },
        qr/Invalid parameter: \"label\" does not exist\./;
    like exception { $ag->update( [ { label => 0, data => { "a" => 1.0, } } ] ); },
        qr/Invalid parameter: \"label\" must be 1 or -1\./;
    like exception { $ag->update( [ { label => 1.0, data => { "a" => 1.0, } } ] ); },
        qr/Invalid parameter: \"label\" must be 1 or -1\./;

    like exception { $ag->update( [ { label => 1, dat => { "a" => 1.0, } } ] ); },
        qr/Invalid parameter: \"data\" does not exist\./;
    like exception { $ag->update( [ { label => 1, data => '' } ] ); },
        qr/Invalid parameter: \"data\" must be HASH-reference\./;
    like exception { $ag->update( [ { label => 1, data => { a => 'test' } } ] ); },
        qr/Invalid parameter: type of internal \"data\" must be number\./;

    $ag->update( [ { label => 1, data => { "a" => 1.0, } }, ] );
